...
const audioList = [
  {
    ...
    url: "/music/我爱过你.mp3",
    cover:
      "http://p1.music.126.net/3hKcOyyd-ufZo4Jl7Lp40Q==/109951164530053133.jpg",
    theme: "#ee0a24",
  },
  {
    name: "静悄悄 (肉麻的调调你不会知道)",
    ...
    url: "/music/静悄悄.mp3",
    cover:
      "http://p1.music.126.net/Ca8woebduPeCzbd9aZzg3g==/109951171419555909.jpg",
    theme: "#ff6b6b",
  },
  {
    name: "Starlight",
    ...
    cover:
      "http://p1.music.126.net/4RbuQ-o572cx1xVBGyz6rg==/109951169721544585.jpg",
    theme: "#4ecdc4",
  },
];
...
export default function MusicPlayer() {
    ...
    const createPlayer = () => {
      if (playerRef.current && window.APlayer && !aplayerRef.current) {
        try {
          ...
          aplayerRef.current.on("canplay", () =>
            setIsPlaying(!aplayerRef.current.audio.paused),
          );

          // 监听歌曲切换
          aplayerRef.current.on("listswitch", (index: any) =>
            setCurrentSong(index.index),
          );

          // 尝试立即自动播放；浏览器通常会拦截带声音的自动播放，
          // 因此在用户首次交互（点击/滚动/触摸/按键）时再兜底触发一次。
          const tryPlay = () => {
            const p = aplayerRef.current?.play?.();
            if (p && typeof p.catch === "function") p.catch(() => {});
          };
          tryPlay();

          const startOnGesture = () => {
            if (aplayerRef.current && aplayerRef.current.audio.paused) {
              tryPlay();
            }
          ...
          gestureEvents.forEach((evt) =>
            window.addEventListener(evt, startOnGesture, {
              once: true,
              passive: true,
            }),
          );
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      <div
        className={`transition-all duration-500 ${isExpanded ? "w-96" : "w-80"}`}
      >
        {/* 头部控制栏 */}
